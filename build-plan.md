...
- [x] **4e5** **There Is No Course, second pass** — six pieces of feedback from Riggs playing it. *(Opus)* — done 2026-08-13.
  ...
  - **`public/media/kayla.mp4` was dropped before launch** ("I think we're dropping the video"), and the written note that had been its fallback is now the ending. The player, the HEAD probe and the branch were removed with it: a code path that can never run is not optionality, it is a skeleton. It cost nothing because the note was written to stand on its own rather than to apologise for something missing.
