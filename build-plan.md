...
| Ch4 | Boost pads, and half-pipes: real boosts that cost you your line | Kart drill with pads and half-pipes; "worth it or not?" quiz cards |
...
- [x] **1b3** Track furniture: real boost pads, ramps with height and a trick-timing window at the lip (hop key at the right moment = landing boost), half-pipes (real ramps with real boosts that throw you off your racing line), collectible coins with counter to 10. *(Opus)* — done 2026-08-07. Also settles Q6: tracks are a plain typed array positioned by `t` around the lap and lateral `offset`.
...
- [x] **1b5** Test track: loop with a hairpin, a U-turn, a chicane, two ramps, real pad strips, one half-pipe, a coin line. *(Sonnet)* — done 2026-08-07. Whole circuit is data in `src/data/test-track.ts`: 17 control points through a closed spline, plus the furniture array. The provisional stadium (`surface.ts`) is deleted as planned.
- [ ] **1b6 GATE (Riggs):** drive 5 minutes. Checklist: the view reads as "Mario Kart-ish" instantly; steering responsive not twitchy; tricks feel snappy; the half-pipe tempts you and the detour is felt; guardrail feels like the real assist; 60fps on a normal Mac. Log measurements and the signed-off config in `TUNING.md`.
...
- [ ] **2b5** Ch4 boost pads and half-pipes: concept + drill: hit every pad, and decide each lap whether the half-pipe is worth it. The lesson is a judgement call, not a trap — a half-pipe gives a genuine trick boost, and the cost is the racing line, so it pays on a straight and loses in a corner. Quiz cards ask "worth it here?" over schematic images rather than "which one is fake". *(Opus)*
